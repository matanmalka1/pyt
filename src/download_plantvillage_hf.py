...
class PlantVillageHFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # HF may store image as PIL or as a dict with 'bytes'
        img = sample["image"]
        ...
        # label may already be an int index
...
def download_and_prepare(cache_dir: str):
    ...
    # Detect class names from the label feature
...
def build_loaders(hf, class_names, img_size, batch_size, workers):
    ...
    HF splits may be named 'train'/'test' — we create a val split if needed.
    """
    ...
    # If no val split, carve 10% from train
    ...
    hf_test = hf.get("test", hf_val)   # reuse val as test if no test split
...
# ─────────────────────────────────────────────────────────────────────────────
def verify_batch(loaders, class_names, device):
    """Pull one batch and print a quick sanity-check."""
...
def main():
    ...
    # 3. Sanity check
    verify_batch(loaders, class_names, device)
    ...
    return loaders, class_names, n_classes   # importable from other scripts
